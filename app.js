...
window.render = function () {
  ...
  filtered.forEach(task => {
    ...
    const html = `
    <div class="group bg-surface-light dark:bg-surface-dark rounded-xl p-6 shadow-card hover:shadow-md transition-all border-l-[6px] ${borderColors[task.priority] || 'border-l-slate-400'} relative overflow-hidden mb-5" id="card-${task.id}">
        <!-- Main Row -->
        <div class="flex justify-between items-start mb-5 cursor-pointer" onclick="window.toggleExpand('${task.id}')">
            <div class="flex flex-col gap-2">
                <span class="text-base uppercase font-bold text-slate-400 tracking-wider" 
                      contenteditable="true" onblur="window.updateCategory('${task.id}', this)" onclick="event.stopPropagation()">
                      ${task.category || 'General'}
                </span>
                <h3 class="text-2xl font-bold text-slate-800 dark:text-white leading-snug ${isDone ? 'line-through opacity-50' : ''}"
                    contenteditable="true" onblur="window.updateTitle('${task.id}', this)" onclick="event.stopPropagation()">
                    ${escapeHtml(task.title)}
                </h3>
            </div>
            
            <button class="w-14 h-14 rounded-full flex items-center justify-center transition-colors shrink-0 ${isDone ? 'text-brand-500 bg-brand-50/10' : 'text-slate-300 hover:text-brand-500 hover:bg-slate-50 dark:hover:bg-slate-700'}"
                    onclick="event.stopPropagation(); window.toggleExpand('${task.id}')">
                 <span class="material-symbols-outlined text-[36px]">${isDone ? 'check_circle' : 'radio_button_unchecked'}</span>
            </button>
        </div>

        <!-- Meta Row -->
        <div class="flex items-center justify-between pt-5 border-t border-slate-50 dark:border-slate-700/50">
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-2 px-4 py-2 rounded-xl ${bgColors[task.priority] || 'bg-slate-100 dark:bg-slate-800'}" onclick="window.cyclePriority(event, '${task.id}')">
                    <span class="material-symbols-outlined text-[20px] fill-1">flag</span>
                    <span class="text-base font-bold uppercase tracking-wide">${task.priority}</span>
                </div>
                <!-- Visiblity Toggle -->
                <button onclick="window.toggleVisibility(event, '${task.id}')" class="text-slate-400 hover:text-brand-500 p-1">
                    <span class="material-symbols-outlined text-[26px]">${isHidden ? 'visibility_off' : 'visibility'}</span>
                </button>
                ${isHidden ? '<span class="text-sm text-slate-500 uppercase font-medium">Hidden</span>' : ''}
            </div>
            <div class="flex items-center gap-5">
                 <span class="text-lg font-bold ${isDone ? 'text-brand-500' : 'text-slate-500'}">${progress}%</span>
